...
def normalize_llvm_type(ty: str) -> str:
    ...
    # Required generator (rare)
    if "at::Generator" in ty:
        return "Generator"
...
def find_candidate(candidates, llvm_name, llvm_params):
    """
    Find the best-matching YAML candidate for a given LLVM function.

    candidates: list of parse_result dicts from native_functions.yaml
    llvm_name: full LLVM function string, e.g.,
               "at::native::masked_fill__quantized_cpu(...) (.0.0.0)"
    llvm_params: list of LLVM param names/types (from parsing), used for sret adjustment
    """
    # Step 0: trivial case
    if len(candidates) == 1:
        return candidates[0]

    # Step 1: adjust for sret (struct return)
    llvm_param_number = len(llvm_params)
    if llvm_params and "sret_" in llvm_params[0]:
        llvm_param_number -= 1

    # Step 2: filter by number of parameters
    filtered = [x for x in candidates if len(x["params"]) == llvm_param_number]
    if len(filtered) == 1:
        return filtered[0]


    # # Step 3: normalize LLVM types
    llvm_types = extract_llvm_param_types(llvm_name)
    # if llvm_params and "sret_" in llvm_params[0]:
    #     llvm_types = llvm_types[1:]

    # Step 4: compare types with filtered candidates
    for cand in filtered:
        cand_types = [p['type'] for p in cand['params']]
        if cand_types == llvm_types:
            return cand

    raise RuntimeError("Unimplemented matching logic")

if __name__ == "__main__":
    func_db = load_func_db()
    llvm_name_map = {}
    ...
    for item in os.listdir("extracted_smt"):
        ...
        llvm_params = data['paths'][0]["funcInfo"]['params']

        candidates = func_db[torch_name]
        cand = find_candidate(candidates, llvm_name, llvm_params)
        llvm_name_map[llvm_name] = cand

    with open("mapping.pickle", "wb") as f:
        pickle.dump(llvm_name_map, f)
