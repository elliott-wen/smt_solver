...
def seq_set(seq, idx, val):
    ...
    Works for Z3 SeqRef (sequence of ints, etc.).
    """
    # prefix = seq[0:idx]
    prefix = Extract(seq, 0, idx - 1)

    # replacement = [val]
    replacement = Unit(val)

    # suffix = seq[idx+1 : end]
    suffix = Extract(seq, idx + 1, Length(seq) - 1)

    return Concat(prefix, replacement, suffix)
    ...
    def get_arg_tensor(self, idx: int):
        if idx not in self._arg_tensors:
            ...
            if arg_info == "Tensor":
                ...
                self._arg_tensors[idx] = Const(f"optional_str_arg_{idx}", SeqSort(StringSort()))
            else:
...
def empty_like_tensor(bm, tensor_size, tensor_dtype):
    return bm.TensorStruct.tensor_structure(tensor_size, tensor_dtype)
...
class FunctionMapper:
    ...
    def apply(self, fn_name: str, ops):
        ...
        if fn_name == "_ZNK3c1013TensorOptions5dtypeESt8optionalINS_10ScalarTypeEE":
            ...
            return If(
                Length(updated_scalar_type) > 0,
                bm.TensorStruct.tensor_structure(
                    bm.TensorStruct.sizes(original_tensor_option),
                    updated_scalar_type[0]
                ),
                original_tensor_option
            )
        ...
        if fn_name == "_ZN2at15get_num_threadsEv":
            return IntVal(1)

        if fn_name == "_ZN2at14get_thread_numEv":
            return IntVal(1)
        ...
        if fn_name == "_ZNK3c108ArrayRefIlE5sliceEmm":
            origin_arr = self.build_expr(ops[0])
            start = self.build_expr(ops[1])
            length = self.build_expr(ops[2])

            subseq = Extract(origin_arr, start, length)  # slice s[start:start+length]
            return subseq
        ...
        if fn_name == "_ZN3c1014isIntegralTypeENS_10ScalarTypeEb":
            stype = self.build_expr(ops[0])  # ops[0] is a ScalarType
            include_bool = self.build_expr(ops[1])  # ops[1] is the bool

            # We need to handle the bool type
            if include_bool.sort().name() != 'Bool':
                include_bool = include_bool != 0
            ...
            return Or(
                ...
                stype == 3,  # Int
                stype == 4,  # Long
                And(include_bool, stype == 8)  # Bool (only if include_bool is True)
            )
        ...
        if fn_name == "_ZN2at20TensorIteratorConfig14resize_outputsEb":
            logging.warning("_ZN2at20TensorIteratorConfig14resize_outputsEb called")
            return self.build_expr(ops[0])
        ...
        if fn_name == "_ZN2at10empty_likeERKNS_6TensorEN3c1013TensorOptionsESt8optionalINS3_12MemoryFormatEE":
            tensor_size = bm.TensorStruct.sizes(self.build_expr(ops[0]))
            # This is incorrect, but ok for now
            logging.warning("Empty like called, not sound implementation")
            tensor_dtype = bm.TensorStruct.dtype(self.build_expr(ops[0]))
            return empty_like_tensor(bm, tensor_size, tensor_dtype)
        ...
        if fn_name == "_ZNK2at18TensorIteratorBase5numelEv":
            tensor_array = self.build_expr(ops[0])
            sizes = bm.TensorStruct.sizes(tensor_array[0])
            numel = IntVal(1)
            for i in range(4):
                idx = IntVal(i)
        ...
        if fn_name == "_ZN3c1014maybe_wrap_dimEllb":
            dim = self.build_expr(ops[0])  # symbolic dim
            dim_post_expr = self.build_expr(ops[1])  # symbolic ndim
            wrap_scalar = BoolVal(True)

            # Wrap negative indices: if dim < 0, add ndim
            wrapped_dim = If(dim < 0, dim + dim_post_expr, dim)

            # If wrap_scalar is True and ndim == 0 (scalar), dim must be 0
            result = If(wrap_scalar & (dim_post_expr == 0), 0, wrapped_dim)
        ...
        if fn_name == "_ZN2at14TensorIterator9binary_opERNS_10TensorBaseERKS1_S4_":
            ...
            third_tensor = self.build_expr(ops[2])
            seq_expr = Empty(SeqSort(bm.TensorStruct))
            # It is incorrect, as the ops[0] size should be different, but on purpose
            seq_expr = Concat(seq_expr, Unit(second_tensor))  # I change to second_tensor on purpose
            seq_expr = Concat(seq_expr, Unit(second_tensor))  # I change to second_tensor on purpose
            seq_expr = Concat(seq_expr, Unit(third_tensor))
            return seq_expr
        ...
        if fn_name == "_ZNK2at6Tensor4viewEN3c108ArrayRefIlEE":
            ...
            updated_size = self.build_expr(ops[1])
        ...
        if fn_name == "_ZNSt8optionalIN3c1010ScalarTypeEEC2IRKS1_TnNSt9enable_ifIX7__and_vISt6__not_ISt7is_sameIS2_NSt9remove_cvINSt16remove_referenceIT_E4typeEE4typeEEES7_IS8_ISt10in_place_tSF_EESt16is_constructibleIS1_JSB_EESt14is_convertibleISB_S1_EEEbE4typeELb1EEEOSB_":
            seq_expr = Empty(SeqSort(IntSort()))
            element = self.build_expr(ops[0])
            seq_expr = Concat(seq_expr, Unit(element))
            return seq_expr
        ...
        if fn_name == "_ZNK3c107Storage10sym_nbytesEv":
            # Todo: Unsound implementation
            return IntVal(16)

        if fn_name == "_ZNK3c107Storage6nbytesEv":
            return IntVal(16)

        if fn_name == "_ZNK2at10TensorBase8itemsizeEv":
            idx = bm.TensorStruct.dtype(self.build_expr(ops[0]))
            return bm.element_sizes_seq[idx]
        ...
        if fn_name == "_ZN2at14TensorIterator20borrowing_nullary_opERKNS_10TensorBaseE":
            input_tensor = self.build_expr(ops[0])
            seq_expr = Empty(SeqSort(bm.TensorStruct))
        ...
        if fn_name == "_ZN2at14TensorIterator19borrowing_binary_opERKNS_10TensorBaseES3_S3_":
            ...
            seq_expr = Concat(seq_expr, Unit(input_tensor_1))
            seq_expr = Concat(seq_expr, Unit(input_tensor_2))
            return seq_expr
        ...
        if fn_name == "_ZNK2at18TensorIteratorBase11input_dtypeEl":
            return bm.TensorStruct.dtype(self.build_expr(ops[0])[1])
        ...
        if fn_name == "_ZN2at6native21get_dtype_from_resultERNS_6TensorESt8optionalIN3c1010ScalarTypeEE":
            ...
            return bm.TensorStruct.dtype(result_param)
        ...
        if fn_name == "_ZNK2at6Tensor6narrowElll":
            original_tensor = self.build_expr(ops[0])
            dim = self.build_expr(ops[1])
            # start = self.build_expr(ops[2])
            length = self.build_expr(ops[3])
            original_sizes = bm.TensorStruct.sizes(original_tensor)  # It is a sequence of tensor size
            new_sizes = seq_set(original_sizes, dim, length)
            return update_tensor_options_sizes(bm, original_tensor, new_sizes)
        ...
        if fn_name == "_ZNK3c108ArrayRefIlE5sliceEm":
            arr = self.build_expr(ops[0])  # the sequence (SeqSort)
            slice_idx = self.build_expr(ops[1])  # symbolic start index

            # Get the length of the sequence
            arr_len = Length(arr)

            # Compute the length of the slice (from slice_idx to the end)
            slice_len = arr_len - slice_idx

            # Symbolically slice the sequence
            sliced_arr = SubSeq(arr, slice_idx, slice_len)
        ...
        if fn_name == "_ZNSt6vectorIN2at6TensorESaIS1_EEixEm":
            arr = self.build_expr(ops[0])  # the sequence (SeqSort)
            arr_idx = self.build_expr(ops[1])  # symbolic start index
            return arr[arr_idx]
        ...
        if fn_name == "_ZN2at6native26searchsorted_scalar_tensorERKN3c106ScalarERKNS1_6DeviceE":
            return empty_like_tensor(bm, Empty(SeqSort(IntSort())), bm.ScalarStruct.dtype(self.build_expr(ops[0])))
        ...
        if fn_name == "_ZNKSt8optionalIN3c106LayoutEEcvbEv":
            return Length(self.build_expr(ops[0])) > 1

        if fn_name == "_ZNRSt8optionalIN3c106LayoutEEdeEv":
            return self.build_expr(ops[0])[0]
        ...
        if fn_name == "_ZN2at7_uniqueERKNS_6TensorEbb":
            ...
            original_tensor = self.build_expr(ops[0])
            seq_expr = Empty(SeqSort(bm.TensorStruct))
            seq_expr = Concat(seq_expr, Unit(original_tensor))
            seq_expr = Concat(seq_expr, Unit(original_tensor))
            return seq_expr
        ...
        if fn_name == "_ZNSt8optionalIlEC2IRlTnNSt9enable_ifIX7__and_vISt6__not_ISt7is_sameIS0_NSt9remove_cvINSt16remove_referenceIT_E4typeEE4typeEEES4_IS5_ISt10in_place_tSC_EESt16is_constructibleIlJS8_EESt14is_convertibleIS8_lEEEbE4typeELb1EEEOS8_":
            seq_expr = Empty(SeqSort(IntSort()))
            input_param = self.build_expr(ops[0])
            seq_expr = Concat(seq_expr, Unit(input_param))
            return seq_expr
        ...
        if fn_name == "_ZNK2at6Tensor12is_same_sizeERKS0_":
            return bm.TensorStruct.sizes(self.build_expr(ops[0])) == bm.TensorStruct.sizes(self.build_expr(ops[1]))
        ...
        if fn_name == "_ZN3c1025SmallVectorTemplateCommonIlvE4backEv":
            # arr = self.build_expr(ops[0])
        ...
        if fn_name == "_ZNSt14numeric_limitsIN3c104HalfEE6lowestEv":
            return RealVal(0)

        if fn_name == "_ZNSt14numeric_limitsIN3c104HalfEE3maxEv":
            return RealVal(128)

        if fn_name == "_ZNSt14numeric_limitsIN3c108BFloat16EE6lowestEv":
            return RealVal(0)

        if fn_name == "_ZNSt14numeric_limitsIN3c108BFloat16EE3maxEv":
            return RealVal(128)

        if fn_name == "_ZNSt14numeric_limitsIlE6lowestEv":
            return IntVal(0)

        if fn_name == "_ZN2at6native9templates11update_fromIdEEll" or fn_name == "_ZN2at6native9templates11update_fromIfEEll" or fn_name == "_ZN2at6native9templates11update_fromIN3c104HalfEEEll" or fn_name == "_ZN2at6native9templates11update_fromIN3c108BFloat16EEEll":
            return self.build_expr(ops[0])

        if fn_name == "_ZN2at6native9templates9update_toIdEEll" or fn_name == "_ZN2at6native9templates9update_toIfEEll" or fn_name == "_ZN2at6native9templates9update_toIN3c104HalfEEEll" or fn_name == "_ZN2at6native9templates9update_toIN3c108BFloat16EEEll":
            return self.build_expr(ops[0])

        if "_ZL7div_rtnIlET_S0_S0_" in fn_name:
            return IntVal(2)
        ...
        for step in reversed(path.get("detail", [])):
            ...
            if cond['inst'] == "call":
                ...
                if cond['ops'][0]['const'] in (
                        ...
                        "_ZN9__gnu_cxxneIPSt17reference_wrapperIKN2at6TensorEESt6vectorIS5_SaIS5_EEEEbRKNS_17__normal_iteratorIT_T0_EESF_",
                        "_ZN9__gnu_cxxneIPN2at6TensorESt6vectorIS2_SaIS2_EEEEbRKNS_17__normal_iteratorIT_T0_EESC_"):
                    continue
            ...
            # Handle switch
            if step.get("isSwitch"):
                excluded_case = step.get("excludedCases")
                if not excluded_case:
                    taken = step.get("taken")
                    solver.add(expr == taken)
                else:
                    constraints = [expr != case_tmp for case_tmp in excluded_case]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        main("extracted_smt/_ZN2at6native30_batch_norm_legit_no_stats_cpuERKNS_6TensorERKSt8optionalIS1_ES7_bdd.json")
